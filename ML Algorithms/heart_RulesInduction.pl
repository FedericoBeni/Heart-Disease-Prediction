...
:- ensure_loaded(heart_attributiDisc4).
:- ensure_loaded(heart_training_setDisc4).
:- ensure_loaded(heart_test_setDisc4).
...
apprendi( Esempi, Classe, []) :-                % Descrizione vuota perchè non ci sono esempi di altre classi
...
classifica(Oggetto,Classe) :- % Oggetto descritto da una lista Att=Val appartiene a Classe
	Classe <== Descrizione, % se Classe Ã¨ una lista di liste Attx=Valx, di cui una Ã¨
...
valuta([no/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica(Oggetto,no), !,      % prevede correttamente l'assenza di malattia
	VNA1 is VNA + 1,
	valuta(Coda,VN,VNA1,VP,VPA,FN,FNA,FP,FPA,NC,NCA).
valuta([yes/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica(Oggetto,yes), !, % prevede correttamente la presenza di malattia
	VPA1 is VPA + 1,
	valuta(Coda,VN,VNA,VP,VPA1,FN,FNA,FP,FPA,NC,NCA).
valuta([yes/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica(Oggetto,no), !,      % prevede erroneamente l'assenza di malattia
	FNA1 is FNA + 1,
	valuta(Coda,VN,VNA,VP,VPA,FN,FNA1,FP,FPA,NC,NCA).
valuta([no/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica(Oggetto,yes), !, % prevede erroneamente la presenza di malattia
	FPA1 is FPA + 1,
